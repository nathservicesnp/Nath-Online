...
assert(!/Rs\.\s?100/.test(text),'Outdated price');
}
assert.equal((await readFile(path.join(root,'CNAME'),'utf8')).trim(),'www.nathonline.com.np');
const home=await readFile(path.join(root,'index.html'),'utf8');assert(home.includes('9779867302353'));assert(home.includes('Rs. 50'));

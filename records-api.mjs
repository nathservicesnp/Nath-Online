...
export async function recordsApi(request,env,url){
 ...
 const record=await env.DB.prepare('SELECT reference,name,phone,service,service_id,service_title,message,status,outcome,internal_note,created_at,updated_at,closed_at,quote_json,paid_paisa FROM enquiries WHERE reference=?').bind(match[1]).first();
 ...
 return download(JSON.stringify({exported_at:new Date().toISOString(),currency:'NPR',amount_unit:'paisa (100 paisa = 1 NPR)',record,status_history:statusHistory,finance_history:financeHistory},null,2),'application/json;charset=utf-8',match[1]+'.json');
}

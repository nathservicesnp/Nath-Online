...
export default {
 async fetch(request,env){const url=new URL(request.url);let response;try{
  ...
  else if(url.pathname==='/admin'||url.pathname.startsWith('/admin/')){const actor=await administrator(request,env);if(!actor)response=new Response('Admin access is not configured or sign-in is required. Contact the site owner.',{status:401,headers:{'Cache-Control':'no-store','Content-Type':'text/plain;charset=utf-8'}});else if(url.pathname.startsWith('/admin/api/'))response=await adminApi(request,env,url,actor,readBody);else if(['GET','HEAD'].includes(request.method))response=await env.ASSETS.fetch(request);else response=json({error:'Method not allowed'},405);}
